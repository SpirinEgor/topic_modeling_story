...
_dataset = YouTrackIssueDataset(DATA_PATH)
...
def version_counts() -> Figure:
    version = [it for i in range(len(_dataset)) for it in _dataset[i][1]]
...
st.json(asdict(_dataset._issues[0]))
...
random_examples = [f"{i + 1}. {_dataset[randint(0, len(_dataset) - 1)][0]}" for i in range(N_RANDOM_EXAMPLES)]
